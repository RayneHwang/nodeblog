    ...
    if (originalUser) {
      ...
      if (!validator.isNull(userName) && userName != originalUser.userName) {
        Users.getByName(userName, function (error, user) {
          ...
          if (user) {
            ep.emit("setting_error", "userName", "Name has been registered");
          } else {
      ...
      ep.emit("setting_error", "submit", "User does not exists");
    }
  });
  ...
  Users.getByEmail(userEmail, function (error, user) {
    ...
    if (user) {
      ...
      if (getActiveKey == activeKey) {
        ...
        ep.emit("active_error", "userEmail", "ActiveKey does not right");
      }
      
    } else {
      ep.emit("active_error", "userEmail", "Email does not exists");
    }
...
function login(req, res, next) {
  ...
  if (validator.isNull(userEmail)) {
    ep.emit("login_error", "userEmail", "又都是能为空！");
    return;
  }
  
  if (validator.isNull(passWord)) {
    ep.emit("login_error", "passWord", "密码不能为空！");
    return;
  }
  ...
  Users.getByEmail(userEmail, function (error, user) {
    ...
    if (user) {
      if (config.auth.checkActive && !user.activeState) {
        ep.emit("login_error", "userEmail", "邮箱未激活！");
        return;
      }
      ...
      tools.bcryptCompare(passWord, user.passWord, function (error, state) {
        ...
        if (state) {
          ...
          ep.emit("login_error", "passWord", "密码错误！");
        }
        
      });
    } else {
      ep.emit("login_error", "userEmail", "邮箱不存在！");
    }
    return;
  });
...
function register(req, res, next) {
  ...
  if (userName.length < 6) {
    ep.emit("register_error", "userName", "用户名过短！");
    return;
  }
  
  if (userName.length > 15) {
    ep.emit("register_error", "userName", "用户名过长！");
    return;
  }
  if (!validator.isEmail(userEmail)) {
    ep.emit("register_error", "userEmail", "邮箱格式错误！");
    return;
  }
  
  if (validator.isNull(passWord) || passWord.length < 6) {
    ep.emit("register_error", "passWord", "密码过短！");
    return;
  }
  
  if (validator.isNull(passWord) || passWord != confirmPassword) {
    ep.emit("register_error", "confirmPassword", "两次输入的密码不一致！");
    return;
  }
  ...
  Users.getByEmail(userEmail, function (error, user) {
    ...
    if (user) {
      ep.emit("register_error", "userEmail", "邮箱已被注册!");
    } else {
  ...
  Users.getByName(userName, function (error, user) {
    ...
    if (user) {
      ep.emit("register_error", "userName", "用户名已被注册!");
    } else {
